...
from Jani_Music.engine._mdb import mongodb
...
async def set_assistant(chat_id):
    from Jani_Music.engine._ubot import assistants
...
async def get_vcnotify(chat_id: int) -> bool:
    doc = await vcnotifydb.find_one({"chat_id": chat_id})
    return bool(doc.get("status")) if doc else False

async def set_vcnotify(chat_id: int, status: bool):
...
async def get_biodetect(chat_id: int) -> bool:
    doc = await biodetectdb.find_one({"chat_id": chat_id})
    return bool(doc.get("status")) if doc else False

async def set_biodetect(chat_id: int, status: bool):
    await biodetectdb.update_one(
        {"chat_id": chat_id},
        {"$set": {"status": status}},
        upsert=True,
    )
